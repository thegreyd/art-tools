...
class FindBugsSweepTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('elliottlib.bzutil.JIRABugTracker.filter_attached_bugs')
    @patch('elliottlib.cli.find_bugs_sweep_cli.FindBugsSweep.search')
    def test_find_bugs_sweep_json(self, search_mock, jira_filter_mock):
        jira_bug = flexmock(
            id='OCPBUGS-1',
            component='OLM',
            status='ON_QA',
        ...
        bugs_by_type = {"image": [jira_bug]}
        flexmock(sweep_cli).should_receive("categorize_bugs_by_type").and_return(bugs_by_type, [])
        ...
        result = runner.invoke(cli, ['-g', 'openshift-4.6', 'find-bugs:sweep', '-o', 'json'])
        if result.exit_code != 0:
            exc_type, exc_value, exc_traceback = result.exc_info
            t = "\n".join(traceback.format_exception(exc_type, exc_value, exc_traceback))
            self.fail(t)
        self.assertEqual(bugs_by_type, json.loads(result.output))
