...
async def olm_bundle_konflux(
        ...
        nvrs: str, only: bool, exclude: str, force: bool, kubeconfig: str, plr_template: str):
    ...
    if plr_template:
        ...
        plr_template_url = constants.KONFLUX_BUNDLE_BUILD_PLR_TEMPLATE_URL_FORMAT.format(owner=plr_template_owner, branch_name=plr_template_branch)
    ...
    try:
        ...
        if record_log_path.exists():
            ...
            for record in records:
                if record['status'] != '0':
                    raise RuntimeError('record.log includes unexpected build_olm_bundle_konflux '
