...
from pyartcd.pipelines.release_gen_payload import UNVIABLE_ASSEMBLY_ERROR_MSG
...
class BuildSyncPipeline:
    ...
    async def _update_nightly_imagestreams(self):
        ...
        rc, _, stderr = await exectools.cmd_gather_async(cmd, check=False, env=os.environ.copy())
        if rc != 0:
            if UNVIABLE_ASSEMBLY_ERROR_MSG in stderr:
